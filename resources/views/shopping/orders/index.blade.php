			...
			@foreach($orders as $order)
				<div class="card m-3">
					...
					<div class="card-block">
						...
						<a href="{{ route('buyer.orders.show', [$store->user->slug, $store->slug, $order->slug]) }}" class="btn btn-primary">Detaljnije</a>
						@if($order->canEdit())
							<a href="{{ route('buyer.orders.edit', [$store->user->slug, $store->slug, $order->slug]) }}" class="btn btn-primary">Izmeni</a>
						@endif
					</div>
				</div>
			@endforeach
