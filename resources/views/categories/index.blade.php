				...
				<thead class="thead-default">
					<tr>
						<th colspan="6" class="text-center">
							<a href="{{ route('stores.categories.create', [$store->id]) }}">Napravi kategoriju</a>
						</th>
					</tr>
					<tr>
						<th class="w-50p">#</th>
				...
				<tbody>
					@foreach($categories as $category)
						<tr>
							...
							<td class="text-center h3"><a href="{{ route('stores.categories.show', [$store->id, $category->id]) }}">{{ $category->name }}</a></td>

							@if($category->parent)
								<td class="text-center h4"><a href="{{ route('stores.categories.show', [$store->id, $category->parent->id]) }}">{{ $category->parent->name or '' }}</a></td>
							@else
								<td class="text-center h4"><i class="fa fa-ban" aria-hidden="true"></i></td>
							@endif

							<td class="text-center">
								<a href="{{ route('stores.categories.products', [$store->id, $category->id]) }}" class="btn btn-primary">
									<i class="fa fa-archive" aria-hidden="true"></i>
								</a>
							</td>

							<td class="text-center">
								<a href="{{ route('stores.categories.edit', [$store->id, $category->id]) }}" class="btn btn-primary">
