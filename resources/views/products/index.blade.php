		...
		<div class="d-flex flex-wrap justify-content-center">
			...
			@foreach($products as $product)
				...
				@component('partials.product')
					@slot('name')
						<a href="{{ route('stores.products.show', [$store->slug, $product->slug]) }}">{{ $product->name }}</a>
					@endslot
					@slot('price')
						{{ $product->price }}
					@endslot
					...
					@endslot
					@slot('links')
						<a href="{{ route('stores.products.edit', [$store->slug, $product->slug]) }}" class="btn btn-primary">Izmeni</a>
						<form action="{{ route('stores.products.destroy', [$store->slug, $product->slug]) }}" method="post">
							{{ csrf_field() }}
							{{ method_field('delete') }}
